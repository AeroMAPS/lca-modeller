"""
Module to create interpolated activities based on parameter values.
This is a modified version of the interpolate_activities function from lca_algebraic,
adding extrapolation capabilities (keeping value from closest activity outside bounds).
"""
...
def _segments_to_piecewise(param, segments):
    ...
    for start, end, val in segments:
        ...
        if start is not None and end is not None:  # intermediate segment
            cond = cond & (param >= start) & (param < end)
            conds.append((val, simplify(cond)))

        if start is None:  # segment before first point
            cond = cond & (param < end)
            conds.append((val, simplify(cond)))

        if end is None:  # segment after last point
            cond = cond & (param >= start)
            conds.append((val, simplify(cond)))

    return simplify(Piecewise(*conds, (0, True)))
...
def interpolate_activities(
    ...
    extrapolate:
        If True then extrapolates regression outside boundaries (behaviour in legacy lca_algebraic).
        Else the activity value will be kept equal to the closest activity when outside bounds.
    ...
    for i, (curr_val, curr_act) in enumerate(sorted_points):
        ...
        if i == 0:  # also add segment for before first point
            if extrapolate:
                segments[curr_act].append(
                    [None, start, (param - next_val) / (curr_val - next_val)]
                )
            else:
                segments[curr_act].append(
                    [None, start, 1.0]
        ...
        if i == len(sorted_points) - 2:  # add a constant segment for after last point
            if extrapolate:
                segments[next_act].append(
                    [end, None, (param - curr_val) / (next_val - curr_val)]
                )
            else:
                segments[next_act].append(
                    [end, None, 1.0]
